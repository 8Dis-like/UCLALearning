...
class BigramLanguageModel(nn.Module):
    ...
    def generate(self, context, max_new_tokens=100):
        ...
        for _ in range(max_new_tokens):
            # Bigram model only looks at the last token
            idx_cond = context[:, -1:]
...
class SingleHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        # As requested by the docstring, we create an *upper* triangular mask of 1s (to mask out future tokens)
    ...
    def forward(self, x):
        ...
        # Scaled dot-product
        wei = q @ k.transpose(-2, -1) * (self.output_key_query_dim ** -0.5)
        # Apply the causal mask (filling upper triangular part with -inf)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        heads = [getattr(self, f"head_{i}")(x) for i in range(self.num_heads)]
...
class LayerNorm(nn.Module):
    ...
    def forward(self, input):
        ...
        # Variance with unbiased=False as requested
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Applying pre-norm architecture with residual connections
...
class MiniGPT(nn.Module):
    ...
    def forward(self, x):
        ...
        # Trim the pre-registered positional encoding to the length of the current sequence
    ...
    def generate(self, context, max_new_tokens=100):
        ...
        for _ in range(max_new_tokens):
            # Constrain the context to the maximum context length of the model
            idx_cond = context[:, -self.config.context_length:]
            logits = self(idx_cond)
            # Focus on the last time step prediction
            logits = logits[:, -1, :]
            probs = F.softmax(logits, dim=-1)
            idx_next = torch.multinomial(probs, num_samples=1)
            context = torch.cat((context, idx_next), dim=1)
