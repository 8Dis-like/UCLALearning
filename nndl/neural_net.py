...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # 1. First Layer (Affine)
    # X is (N, D), W1 is (H, D) -> W1.T is (D, H)
    z1 = X.dot(W1.T) + b1  # (N, H)

    # 2. Activation (ReLU)
    a1 = np.maximum(0, z1)  # (N, H)

    # 3. Second Layer (Affine)
    # a1 is (N, H), W2 is (C, H) -> W2.T is (H, C)
    scores = a1.dot(W2.T) + b2  # (N, C)
    # ================================================================ #
    ...
    # 1. Gradient of Softmax Loss
    dscores = probs
    dscores[np.arange(N), y] -= 1
    dscores /= N

    # 2. Backprop into W2 and b2
    # dscores is (N, C), a1 is (N, H)
    # dW2 = dscores^T * a1 -> (C, N) * (N, H) = (C, H)
    grads['W2'] = dscores.T.dot(a1) + reg * W2
    grads['b2'] = np.sum(dscores, axis=0)

    # 3. Backprop into hidden layer
    # da1 = dscores * W2 -> (N, C) * (C, H) = (N, H)
    da1 = dscores.dot(W2)

    # 4. Backprop through ReLU
    dz1 = da1
    dz1[a1 <= 0] = 0  # ReLU gradient is 0 where input was <= 0

    # 5. Backprop into W1 and b1
    # dz1 is (N, H), X is (N, D)
    # dW1 = dz1^T * X -> (H, N) * (N, D) = (H, D)
    grads['W1'] = dz1.T.dot(X) + reg * W1
  ...
  def predict(self, X):
    ...
    a1 = np.maximum(0, z1)
    scores = a1.dot(self.params['W2'].T) + self.params['b2']
